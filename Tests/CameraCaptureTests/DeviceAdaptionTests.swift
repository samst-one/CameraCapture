...
class AdaptionTests: XCTestCase {
    ...
    func testScaleToMultiplication() {
        XCTAssertEqual(ZoomScaleToMultiplication.adapt(deviceType: .dualCamera, scale: 2), 2.0)
    ...
    func testMultiplicationToScale() {
        XCTAssertEqual(ZoomMultiplcationToScale.adapt(deviceType: .dualCamera, multiplier: 2), 2.0)
